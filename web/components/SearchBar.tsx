...
export default function SearchBar({
  ...
  return (
    <div ref={boxRef} className="relative w-full">
      ...
      >
        {/* поле услуги */}
        <div className={`relative flex flex-1 items-center rounded-xl border border-line2 bg-surface sm:border-0 ${pad}`}>
        ...
        {/* город */}
        <div className={`relative flex items-center rounded-xl border border-line2 bg-surface sm:border-0 sm:border-l sm:border-line sm:bg-transparent ${pad}`}>
          <CityPicker
            value={city}
            cities={cities}
            onChange={setCity}
            triggerClassName="flex h-full min-w-[150px] cursor-pointer items-center gap-2 bg-transparent px-3.5 text-left outline-none sm:px-3"
          />
        </div>

        <button
          type="submit"
          className={`shrink-0 rounded-xl bg-brand px-7 font-semibold text-white transition-colors hover:bg-brand-ink ${pad}`}
        >
          Найти
        </button>
      </form>
